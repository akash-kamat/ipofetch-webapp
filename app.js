...
function showDetail(ipo) {
  ...
  $("#detail-content").innerHTML = `<div class="dialog-header"><h2>${esc(ipo.companyName)}</h2><p>Complete normalized API record</p></div><div class="detail-grid">${fields.map(([label, value]) => `<div class="detail"><span>${esc(label)}</span><strong>${esc(valueOrDash(value))}</strong></div>`).join("")}</div>${subscriptionSection}<div class="dialog-foot"><span>Estimated listing = upper price band + current GMP; it is not a forecast.</span>${url ? `<a href="${esc(url)}" target="_blank" rel="noreferrer">InvestorGain GMP source ↗</a>` : ""}</div>`;
